...
describe('TokenModel', () => {
  ...
  const user = {
    id: 5,
    username: 'loftdigital',
    email: 'test@loftdigital.com',
    enabled: true,
    plain_password: null,
    first_name: 'Loft',
    last_name: 'Digital',
    created_at: '2016-02-25T09:38:08+0000',
  };
  const decoded = {
    exp: 1458685038,
    username: 'loftdigital',
  ...
  describe('set', () => {
    it('should set the token', () => {
      ...
      expect(tokenModel.decoded).to.deep.equal(decoded);

      expect(stub).to.have.been.calledOnce;
      expect(stub).to.have.been.calledWithMatch(decoded.id);
  ...
  describe('updateCurrentUser', () => {
    it('should update our current user store', () => {
      ...
      return expect(tokenModel.updateCurrentUser()).eventually.to.deep.equal(user);
    });
