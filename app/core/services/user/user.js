...
class UserService {
  ...
  /**
   * Handles the user request to update their details
   *
   * @param {Object} user - The user object
   * @param {string} self - Reference to the forms 'this' value
   * @param {string} form - The submitted form
   * @returns {Promise} The request promise
   */
  update(user, self, form) {
    return this.UsersModel.save(user)
      .then(response => {
        this.onUpdateSuccess(self);
        return Promise.resolve(response);
      }, response => {
        this.onUpdateFailure(self, response);
